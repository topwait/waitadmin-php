...
let waitUtil = {
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                error: function(jqXHR, error, errorThrown) {
                    layer.msg('请求错误:'+error, {icon: 2, time: 1500}, function () {
                        reject(error);
                    });
                },
    ...
    ,uploader: function (options) {
        ...
        return new Promise((resolve) => {
            ...
            let route = '/' + paths + 'attach/index?type=' + typeNum;
            parent.layer.open({
                type: 2
                ,title: '选择附件'
                ,shadeClose: true
    ...
    builder.each(function () {
        ...
        switch (type||'image') {
            case 'image':
                ...
                url = '/'+pathname+'/upload/temporary?type=image';
                break;
            case 'video':
                input = 'video';
                accept = 'video';
                ...
                url = '/'+pathname+'/upload/temporary?type=video';
                break;
            case 'package':
                break;
            case 'document':
 ...
 * 按钮下拉
 */
function layTableButton() {
    $('.layui-btn-drop .toggle').click(function (e) {
        e.stopPropagation();
        if ($(this).hasClass('active') === true) {
            ...
            $('.layui-btn-drop .toggle').removeClass('active');
            $('.layui-btn-drop .dropdown-menu').hide();
            $(this).addClass('active');
            $(this).next().css({'top': top + 'px', 'left': left + 'px'});
            $(this).next().show();
            return false;
        }
    });
    ...
    $(window).resize(function() {
        if (ttl === false) {
            ttl = true;
            setTimeout(function () {
                let nodes = $('.layui-btn-drop .toggle.active');
                if (nodes.length) {
                    let alias   = nodes.parent().attr('data-alias');
                    let trIndex = nodes.parents('tr').attr('data-index');
                    let tdIndex = nodes.parents('td').attr('data-field');
                    let isMains = nodes.parents('table').parent().hasClass('layui-table-main');
                    let isFixed = $('.layui-table-view .layui-table-fixed').hasClass('layui-hide');

                    let elem = 'tr[data-index='+trIndex+'] td[data-field='+tdIndex+']';
                    let pans = alias ? ' .layui-btn-drop[data-alias='+alias+']' : ' .layui-btn-drop';
                    let drop = null;
                    if (!isFixed) {
                        drop = $('.layui-table-view .layui-table-fixed ' + elem + pans);
                        if (isMains) {
                            nodes.removeClass('active');
                            nodes.next().hide();
                        }
                    } else {
                        drop = $('.layui-table-view .layui-table-main ' + elem + pans);
                        hide = $('.layui-table-view .layui-table-fixed ' + elem + pans);
                        hide.children('.toggle').removeClass('active');
                        hide.children('.dropdown-menu').hide();
                    }

                    let top  = drop.offset().top + 30;
                    let left = drop.offset().left.toFixed(2);
                    drop.children('.toggle').addClass('active');
                    drop.children('.dropdown-menu').show();
                    drop.children('.dropdown-menu').css({'top': top + 'px', 'left': left + 'px'});
                }
                ttl = false;
            }, 100);
        }
