...
let waitUtil = {
    ...
    ,pages: function (options) {
        const laySkin   = options.skin || 'popup-skin-window';
        const layOpen   = options.parent ? parent.layer : layer;
        const loadIndex = layOpen.load(2);
        layOpen.open({
            type: options.type || 2
            ...
            ,success: function(layero, index) {
                ...
                if (options.area && options.area.length < 2) {
                    let iframe = $layero.find('iframe');
                    let iframeHeight = iframe[0].contentDocument.body.offsetHeight;
                    let windowElemNo = options.parent ? parent.window.innerHeight : window.innerHeight;
                    if (iframeHeight > windowElemNo) {
                        iframeHeight = windowElemNo - 50;
                    }
                    iframe.css('height', iframeHeight);
                    $layero.css('top', (windowElemNo - iframe[0].offsetHeight) / 2);
                }

                layOpen.close(loadIndex);
