        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (options.that) {
                        options.that.removeClass('layui-btn-forbid');
                    }
                    resolve(res);
                },
            ...
            ,parseData: function(res){
                ...
                setTimeout(function () {
                    ...
                    $('.dropdown-btn-group').hover(function () {
                        let top = $(this).offset().top + 28;
                        let left = $(this).offset().left - 27;
                        $(this).children('dl').css({'top': top+'px', 'left': left+'px'})
                    });
...
function laySession() {
    const login = () => {
        layer.open({
            ...
            skin: 'wait-empty',
            area: ['370px'],
            content: '/frontend/login/index?scene=login',
            success: function(layero, index) {
                let iframeElement = $(layero).find("iframe").contents();
                iframeElement.find(".signup").click(function(){
                    layer.close(index)
                    register()
                });
            }
    ...
    const register = () => {
        layer.open({
            ...
            skin: 'wait-empty',
            area: ['370px'],
            content: '/frontend/login/index?scene=register',
            success: function(layero, index) {
                let iframeElement = $(layero).find("iframe").contents();
                iframeElement.find(".enroll-k").click(function(){
                    layer.close(index);
                    login();
                });
            }
    ...
    $(document).on('click', '#js-enroll-btn', () => { login() });
    $(document).on('click', '#js-signup-btn', () => { register() });
}
...
function layThumbUpload() {
    ...
    builder.each(function () {
        ...
        let type   = node.attr('data-type') || 'image';
        ...
        switch (type) {
            ...
            case 'video':
                ...
                break;
            case 'zip':
                reqUrl = '/frontend/upload/zip';
                accept = 'file';
                acceptMime = '.zip';
                exts = 'zip';
                size = 1024 * 10;
                field = field === undefined ? 'zip' : field;
        }
        ...
        function getTemplate(type, url) {
            switch (type) {
                ...
                case 'video':
                    ...
                    break;
                case 'zip':
                    return '<img src="/static/frontend/images/zip.png">';
                    break;
            }
        }
...
function layThumbSelect() {
    $(document).on('click', '.thumbnail .builder.layui-auto', function () {
        let that   = $(this);
        let elem   = $(this).parents('.thumbnail');
        ...
        return new Promise((resolve) => {
            parent.layer.open({
                ...
                ,shade: 0.3
                ,area: [width, height]
                ,content: '/admin/attach/index?type='+type
                ,success: function (layero, index) {
