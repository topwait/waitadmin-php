 ...
 */
export function toDateFormat(timestamp, format = 'YY-MM-DD hh:mm:ss') {
    if (String(timestamp).length <= 10) {
        timestamp = timestamp * 1000
    }
 ...
 * 日期格式转时间戳
 * 
 * @param {Object} datetime 日期时间
 * @return {String} 格式化后的时间
 */
export function toTimeFormat(datetime) {
...
export function week() {
    ...
    let startTime = toTimeFormat(formatDate(startDate))
    let endTime   = toTimeFormat(formatDate(endDate))
