...
const isDisabled = computed(() => {
    return locking.value || !!errorTips.value || !formData.account
})
...
const handleSubmit = async () => {
    const account: string = userStore.userInfo.account
    if (formData.account === account) {
        toast.show('与原账号一致,无需修改')
        return false
    }
...
watch(
    ...
    (val: string) => {
        if (!val) {
            errorTips.value = '账号不能为空'
        }  else if (!/^[a-zA-Z0-9]+$/.test(val)) {
            errorTips.value = '账号只能包含字母或数字'
        } else if (val.length <= 1 || val.length > 20) {
            errorTips.value = '账号长度不合法，应为2-20个字符'
        } else {
            errorTips.value = ''
        }
    }
